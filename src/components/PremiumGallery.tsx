...
export function PremiumGallery() {
  const categories = useMemo(() => ["Todos", ...Array.from(new Set(propertyData.gallery.map((item) => item.category)))], []);
    ...
    <section id="galeria" className="bg-sand py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        <div className="mb-10 md:flex md:items-end md:justify-between">
          ...
          <p className="mt-4 max-w-sm text-sm leading-6 text-forest/65 md:mt-0">Fotos reais da Casa da Floresta, do interior ao deck cercado pela Mata Atlântica.</p>
        </div>
        ...
        <div className="-mx-5 mb-9 flex gap-2 overflow-x-auto px-5 pb-2 scrollbar-none md:mx-0 md:px-0" role="tablist" aria-label="Filtrar galeria">
          {categories.map((category) => (
            <button
              ...
              className={`shrink-0 rounded-full border px-4 py-2 text-sm transition-colors ${filter === category ? "border-forest bg-forest text-white" : "border-forest/20 bg-transparent text-forest hover:border-forest/50"}`}
              role="tab"
        ...
        <div className="grid auto-rows-[230px] grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:auto-rows-[260px]">
          {visible.map((item, index) => (
            <button
              ...
              className={`group relative overflow-hidden rounded-[1.5rem] text-left ${item.shape === "tall" ? "sm:row-span-2" : item.shape === "wide" ? "lg:col-span-2" : ""}`}
            >
              ...
              <div className="absolute inset-0 bg-forest/20 transition-colors group-hover:bg-forest/30" />
              <div className="absolute inset-x-3 bottom-3 flex items-end justify-between rounded-2xl bg-forest/75 p-4 text-white backdrop-blur-sm">
                <div>
                  <span className="text-[10px] uppercase tracking-[.2em] text-white/70">{item.category}</span>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-5xl overflow-hidden border-0 bg-forest p-0 text-white sm:rounded-[2rem] [&>button]:hidden">
          ...
          {selected && <img src={selected.src} alt={selected.label} className="max-h-[80vh] min-h-[55vh] w-full object-contain bg-[#132c22]" />}
          <div className="absolute inset-x-4 bottom-4 flex items-end justify-between rounded-2xl bg-forest/85 p-5 backdrop-blur-sm">
            <div>
              <p className="text-xs uppercase tracking-[.2em] text-white/70">{selected?.category}</p>
              <p className="mt-1 font-serif text-2xl">{selected?.label}</p>
            </div>
